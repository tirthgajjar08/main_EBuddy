...
function ProductListing() {
  ...
  function setDataTOStorage(id, product_id, seller_name, sku_id, status, stock, dilvery_charges, category_name, sub_category_name, product_brand, product_name, product_image, product_mrpprice, product_price, ram, storage, color, display, processor, os, camera, battery, product_details, create_date) {
  ...
  const handleFilter = (e) => {
    ...
    const filteredData = prodcutsValue.filter((item) => {
      const isProductBrandMatch = item.product_brand.toLowerCase().includes(inputValue);
      const isProductNameMatch = item.product_name.toLowerCase().includes(inputValue);
      const isRamMatch = item.ram.toLowerCase().includes(inputValue);
      const isCategoryNameMatch = item.category_name.toLowerCase().includes(inputValue);
        ...
        <div className='row prolist_up_style'>
          <div className='col-4'>
            <div className='prolist_active_style'>
              <h3><BsDatabaseFillCheck style={{ fontSize: "29px", marginRight: "13px" }} />25</h3>
                    ...
                    <tbody>
                      {
                        filterData.length === 0 ? (
                          <tr>
                            <td></td>
                            <td colSpan="10" style={{margin:"80rem"}}>No Data found  !  Try again another Detail.</td>
                          </tr>
                        ) : (
